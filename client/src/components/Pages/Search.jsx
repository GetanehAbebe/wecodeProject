    ...
    const getResults = () => {
        ...
        if (selectedIngredients.length) result = result.filter(item => {
            for (const prop of selectedIngredients) {
                ...
                console.log('ingredints', item.ingredients)
                for (let cat of item.ingredients) {
                    if (prop.name == cat.ingredient || cat.name) return true
                }
            }
        })
                    ...
                    {recipes.length > 0 && recipes.slice(pageNumber * 5, (pageNumber + 1) * 5).map((recipe, i) => (
                        <div class="recipe-card recipe" key={i}>
                            <Link to={`products/${recipe.id}`}>
                                <div class="image">
                                    <img src={recipe.images[0] ? `http://localhost:3200/${recipe.images} ` : 'https://images.pexels.com/photos/3843282/pexels-photo-3843282.jpeg?auto=compress&cs=tinysrgb&dpr=3&h=750&w=1260'} />
                                    <div >
